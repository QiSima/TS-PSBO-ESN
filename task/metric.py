...
def wmape(target, pred, ith=None):
    '''
    target : np.array(n_samples, n_outputs)\n
    pred   : np.array(n_samples, n_outputs)
    '''
    def h_wmape(target, pred, ith):
        t, p = target[:, ith-1], pred[:, ith-1]
        
        mask = t != 0.0
        # MAPE
        error_ith = (t[mask] - p[mask]) / t[mask] * 100
        # Weight
        error_ith = np.multiply(np.fabs(error_ith),np.fabs(t[mask]))
        error_ith = np.sum(error_ith)/np.sum(np.fabs(t[mask]))
        _e_h = error_ith.item()
        return _e_h
    ...
    if ith is None:
        ...
        errors = np.zeros(H)
        for h in range(H):
            errors[h] = h_wmape(target, pred, h+1)
        output = errors.mean().item()
    else:
        output = h_wmape(target, pred, ith)
    return output

def wape(target, pred, ith=None):
    '''
    target : np.array(n_samples, n_outputs)\n
    pred   : np.array(n_samples, n_outputs)
    '''
    def h_wape(target, pred, ith):
        t, p = target[:, ith-1], pred[:, ith-1]
        
        mask = t != 0.0
        # numerator
        error_ith = np.sum(np.fabs(t[mask] - p[mask]))
        # denominator 
        denominator_ith = np.sum(np.fabs(t))
        _e_h = error_ith/denominator_ith.item() * 100
        return _e_h
    ...
    if ith is None:
        ...
        errors = np.zeros(H)
        for h in range(H):
            errors[h] = h_wape(target, pred, h+1)
        output = errors.mean().item()
    else:
        output = h_wape(target, pred, ith)
    return output

def mape100(target, pred, ith=None):
    '''
    target : np.array(n_samples, n_outputs)\n
    pred   : np.array(n_samples, n_outputs)
    '''
    output = mape(target, pred, ith=ith)
    output = output * 100
    return output
...
def smape(target, pred, ith=None):
    ...
    def h_smape(target, pred, ith):
        ...
        mask = t != 0.0
        error_ith = np.fabs(t[mask] - p[mask]) / (np.fabs(t[mask]) + np.fabs(p[mask]))
        error_ith = np.fabs(error_ith)
        _e_h = error_ith.mean().item()
        return _e_h
...
def smape200(target, pred, ith=None):
    '''
    target : np.array(n_samples, n_outputs)\n
    pred   : np.array(n_samples, n_outputs)
    '''
